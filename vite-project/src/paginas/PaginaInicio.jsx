...
// uso state y effect para traer productos de la API y mostrar las ofertas en el carrusel

function PaginaInicio() {
  // useNavigate me permite navegar a otras rutas
  const navigate = useNavigate()
  // productos: alamceno todos los productos que devuelve la API
  const [productos, setProductos] = useState([])
  // loadingOfertas: indica si todavia se están cargando los productos para el carrusel
  ...
  return (
    <div className="min-h-screen flex justify-center">
      <div className="w-full max-w-5xl px-6 py-6 flex flex-col gap-4">
        ...
        <nav aria-label="Categorías principales" className="flex justify-center gap-3 py-2">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <article className="flex flex-col gap-3 p-3">
              <div className="w-full rounded-2xl overflow-hidden border border-white/20 cursor-pointer" onClick={() => navigate('/categoria/hombre')}>
                <img src={imagenBannerHombre} alt="Categoría hombre" className="w-full h-full object-cover"/>
              </div>
                ...
                {productosOferta.map((producto) => {
                  ...
                  return (
                    <article key={producto.id} className="min-w-[220px] rounded-xl border border-white/20 bg-black/30 p-3 flex flex-col gap-2">
                      {/* imagen y badge rojo de descuento */}
                      <div className="relative h-36 rounded-lg border border-white/25 overflow-hidden flex items-center justify-center bg-black/40">
                        ...
                        {imagen ? (
                          <img src={imagen} alt={producto.title} className="w-full h-full object-contain" />
                        ) : (
                      ...
                      {/* precio original tachado y precio final con descuento */}
                      ...
                      {/* boton que te lleva al detalle del producto */}
        ...
        <footer className="mt-4 grid grid-cols-1 md:grid-cols-[2fr_1.2fr] gap-4 border-t border-white/10 pt-4">
          <section>
            ...
            <p className="text-sm text-gray-300 mt-1">Dirección: 1-23-4 Jinnan, Shibuya City, Tokyo 150-0041, Japan.</p>
            <p className="text-sm text-gray-300">Email: contact@mat-jp.co</p>
            <p className="text-sm text-gray-300">Teléfono: +81 3-5555-0101</p>
            <p className="text-sm text-gray-300">Horario de Atención: Never Sleep, 24/7.</p>
          </section>
        </footer>
      </div>
    </div>
