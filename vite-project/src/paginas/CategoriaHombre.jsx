...
// pagina de categoria para productos etiquetados como "Hombre"
function CategoriaHombre() {
  const navigate = useNavigate()

  // productos guarda todo lo que trae la API
  const [productos, setProductos] = useState([])
  // loading indica si todavía se está cargando la lista
  const [loading, setLoading] = useState(true)
  // filtroCategoria filtra por categoría general (todos, calzado, indumentaria y accesorios)
  const [filtroCategoria, setFiltroCategoria] = useState('todos')

  // pido los productos a la API al armar el componente
  useEffect(() => {
    async function cargarProductos() {
      try {
        ...
        if (!respuesta.ok) {
          console.error('Error al pedir productos hombre:', respuesta.status)
          setProductos([])
          return
        }

        const data = await respuesta.json()
        console.log('Respuesta productos hombre:', data)
        ...
        console.error('Error de red al cargar productos hombre:', error)
        setProductos([])
      } finally {
        setLoading(false)
      }
  ...
  // primer filtro, solo productos que tengan el tag "Hombre"
  const productosHombre = productos.filter((producto) => {
    if (!producto.tags || !Array.isArray(producto.tags)) return false
    return producto.tags.some((tag) => tag.title === 'Hombre')
  })

  // segundo filtro, por categoria general (Calzado o Indumentaria o Accesorios)
  ...
  // arma la URL de la imagen principal del producto, null si no tiene
    ...
    <div className="min-h-screen flex justify-center">
      <div className="w-full max-w-5xl px-6 py-6 flex flex-col gap-4">
        ...
        <nav className="flex justify-center gap-3 py-2">
          <button type="button" onClick={() => setFiltroCategoria('todos')} className={`px-4 py-1 rounded-full border border-white/20 text-xs md:text-sm transition ${
              filtroCategoria === 'todos' ? 'bg-white/20' : 'hover:bg-white/10' }`} >Todos</button>

          <button type="button" onClick={() => setFiltroCategoria('calzado')} className={`px-4 py-1 rounded-full border border-white/20 text-xs md:text-sm transition ${
              filtroCategoria === 'calzado' ? 'bg-white/20' : 'hover:bg-white/10' }`} >Calzado</button>

          <button type="button" onClick={() => setFiltroCategoria('indumentaria')} className={`px-4 py-1 rounded-full border border-white/20 text-xs md:text-sm transition ${
              filtroCategoria === 'indumentaria' ? 'bg-white/20' : 'hover:bg-white/10' }`} > Indumentaria </button>

          <button type="button" onClick={() => setFiltroCategoria('accesorios')} className={`px-4 py-1 rounded-full border border-white/20 text-xs md:text-sm transition ${
              filtroCategoria === 'accesorios' ? 'bg-white/20' : 'hover:bg-white/10' }`} > Accesorios </button>
        </nav>

        {/* listado de productos para la categoria */}
        <main className="flex flex-col gap-4">
          <h1 className="text-xl md:text-2xl font-semibold">Productos Hombre</h1>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {productosFiltrados.map((producto) => {
                ...
                return (
                  <article
                    key={producto.id}
                    className="rounded-2xl border border-white/20 bg-black/30 p-3 flex flex-col gap-2"
                  >
                    {/* Imagen del producto y badge de oferta si es q tiene*/}
