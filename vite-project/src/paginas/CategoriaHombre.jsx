...
// pagina de categoria para productos "Hombre"
function CategoriaHombre() {
  ...
  // loading indica si todavia se esta cargando la lista
  const [loading, setLoading] = useState(true)
  // filtroCategoria filtra por categoria general
  const [filtroCategoria, setFiltroCategoria] = useState('todos')
  // controla el panel lateral del carrito
  const [carritoAbierto, setCarritoAbierto] = useState(false)

  // pido los productos a la API al montar el componente
  useEffect(() => {
  ...
  // segundo filtro, por categoria general
  ...
  // arma la URL de la imagen principal del producto
                ...
                return (
                  <article key={producto.id} className="rounded-2xl border border-white/20 bg-black/30 p-3 flex flex-col gap-2">
                    <div className="relative w-full aspect-square rounded-xl overflow-hidden border border-white/20 flex items-center justify-center">
                      ...
                      {imagen ? <img src={imagen} alt={producto.title} className="w-full h-full object-cover" /> : <span className="text-xs text-gray-300">Sin imagen</span>}
                    </div>
                    ...
                    <button type="button" onClick={() => agregarProducto(producto)} className="mt-2 w-full px-3 py-2 rounded-full bg-white/20 hover:bg-white/30 text-xs md:text-sm transition">Agregar al carrito</button>
                    <button type="button" onClick={() => navigate(`/producto/${producto.id}`)} className="mt-2 w-full px-3 py-2 rounded-full bg-white/15 hover:bg-white/25 text-xs md:text-sm transition">Ver detalle</button>
                  </article>
                )
              })}
            </section>
